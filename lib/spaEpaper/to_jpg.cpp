...
static IRAM_ATTR void convert_line_format(uint8_t *src, newpixformat_t format, uint8_t *dst, size_t width, size_t in_channels, size_t line)
{
    int i = 0, o = 0, l = 0;
    if (format == NEW_PIXFORMAT_GRAYSCALE)
    {
        memcpy(dst, src + line * width, width);
    }
    else if (format == NEW_PIXFORMAT_GRAYSCALE16)
    {
        // Log.verbose("Converting GRAYSCALE16 width %d, line %d\n", width, line);
        // memcpy(dst, src + line * width, width);
        for (int i = 0; i < width / 2; i++)
        {
            dst[i * 2 + 1] = (src[i + line * width / 2] >> 4) * 0x11;
            dst[i * 2] = (src[i + line * width / 2] & 0x0F) * 0x11;
            // dst[i * 2 + 1] = 0xff;

            // dst[i * 2] = (src[i + line * width / 2] >> 4) * 0x11;
        }
    }
    else if (format == NEW_PIXFORMAT_RGB888)
    {
        l = width * 3;
        src += l * line;
        for (i = 0; i < l; i += 3)
    ...
    else if (format == NEW_PIXFORMAT_RGB565)
    {
        l = width * 2;
        src += l * line;
        for (i = 0; i < l; i += 2)
    ...
    else if (format == NEW_PIXFORMAT_YUV422)
    {
        uint8_t y0, y1, u, v;
        uint8_t r, g, b;
        l = width * 2;
...
bool convert_image(uint8_t *src, uint16_t width, uint16_t height, newpixformat_t format, uint8_t quality, jpge::output_stream *dst_stream)
{
    int num_channels = 3;
    jpge::subsampling_t subsampling = jpge::H2V2;

    if (format == NEW_PIXFORMAT_GRAYSCALE || format == NEW_PIXFORMAT_GRAYSCALE16)
    {
        num_channels = 1;
...
bool fmt2jpg_cb(uint8_t *src, size_t src_len, uint16_t width, uint16_t height, newpixformat_t format, uint8_t quality, jpg_out_cb cb, void *arg)
{
