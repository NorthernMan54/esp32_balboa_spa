...
void ui_init_old(void)
{
  ...
  for (i = 0; i < 6; i++)
  {
    ...
    if (row == 0)
    {
      if (col == 0)
      {
        log_i("thermostatLine i=%lu, col=%d, row=%d, ", i, col, row);
        obj = thermostatLine(cont);
      }
      else
      {
        log_i("thermostatArc i=%lu, col=%d, row=%d, ", i, col, row);
        obj = thermostatArc(cont);
      }
    }
    else if (row == 1)
    {
 ...
 */
lv_obj_t *thermostatLine(lv_obj_t *parent)
{
